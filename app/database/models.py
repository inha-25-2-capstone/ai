...
class Topic(Base):
    ...
    __table_args__ = (
        CheckConstraint('topic_rank >= 1 AND topic_rank <= 7', name='check_topic_rank'),
    )
...
class RecommendedArticle(Base):
    ...
    def __repr__(self):
        return f"<RecommendedArticle(topic_id={self.topic_id}, article_id={self.article_id}, stance='{self.stance_type}')>"
