...
from app.api.schemas import (
    AnalyzeRequest, StanceResult, BatchAnalyzeRequest, BatchAnalyzeResponse,
    TopicAnalyzeRequest, TopicAnalyzeResponse, SaveStanceRequest, BatchSaveStanceRequest,
    HealthResponse, ErrorResponse
)
from app.services import StanceService
from app.database import get_db, StanceAnalysis, Article
...
def get_stance_service():
    ...
    if stance_service is None:
        raise HTTPException(
            ...
            detail="Stance service not initialized"
        )
...
async def analyze_articles_batch(
    ...
    try:
        ...
        results = service.analyze_articles_batch(
            articles,
            batch_size=request.batch_size
        )

        return BatchAnalyzeResponse(
            results=[StanceResult(**result) for result in results]
        )
...
async def save_stance_analysis(
    request: SaveStanceRequest,
    ...
    try:
        ...
        # 기존 분석 결과 확인
        existing_analysis = db.query(StanceAnalysis).filter(
            StanceAnalysis.article_id == request.article_id
        ).first()
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="스탠스 분석 결과 저장 중 오류가 발생했습니다."
        )
...
async def save_stance_analysis_batch(
    request: BatchSaveStanceRequest,
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        for analysis in request.analyses:
            ...
            # 기존 분석 결과 확인
            existing_analysis = db.query(StanceAnalysis).filter(
                StanceAnalysis.article_id == analysis.article_id
            ).first()
...
async def get_stance_analysis(
    ...
    """
    try:
        analysis = db.query(StanceAnalysis).filter(
            StanceAnalysis.article_id == article_id
        ).first()

        if not analysis:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Stance analysis for article {article_id} not found"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="스탠스 분석 결과 조회 중 오류가 발생했습니다."
        )
